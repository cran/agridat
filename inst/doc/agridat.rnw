% agridat.rnw
% Time-stamp: c:/x/agridat.rnw

\documentclass[11pt]{article}

% \VignetteIndexEntry{Some examples from 'agridat'}
% \VignetteEngine{knitr::knitr}

%\RequirePackage{Bioconductor}
\RequirePackage{agridat}

\begin{document}

\title{The agridat package is growing}
\author{Kevin Wright}
\maketitle
\thispagestyle{fancy}
...
Do you think the model is a good fit to the data?
...
10 years.  Each panel shows a scatterplot and regression for yield verses the
...
\section*{Genotype + Genotype * Block Biplot}
...
block/group of environments.  In the two examples below, each location is
...
For the two data sets below, does either axis 1 or axis 2 show genetic
differences, or only interactions?
